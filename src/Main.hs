...
startingPlayersMiddle ctr1 ctr3 ctr2 ctr4 = 
    [ctr1 "X" (middleColumn, firstRow) wallsPerPlayer winningX,
     ctr3 "W" (firstColumn, middleRow) wallsPerPlayer winningW, 
...
main :: IO ()
main = do {
    putStrLn "What kind of player is player X? (Human/Dumb/Minimax)";
    playerX<-getLine;
    putStrLn "What kind of player is player W? (Human/Dumb/Minimax)";
    playerW<-getLine;
    putStrLn "What kind of player is player Y? (Human/Dumb/Minimax)";
    playerY<-getLine; 
    putStrLn "What kind of player is player Z? (Human/Dumb/Minimax)";
    playerZ<-getLine;
    case (nameToPlayerConstructor playerX, nameToPlayerConstructor playerZ, nameToPlayerConstructor playerY, nameToPlayerConstructor playerW) of 
        (Just ctrX,  Just ctrW, Just ctrY, Just ctrZ) -> do { play (Game startingBoard (startingPlayersMiddle ctrX ctrW ctrY ctrZ)) }
